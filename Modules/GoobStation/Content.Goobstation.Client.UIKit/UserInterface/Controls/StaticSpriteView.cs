...
public class StaticSpriteView : Control
{
    ...
    private Entity<SpriteComponent>? _cachedSprite;
    ...
    /// applying the explicitly set scale <see cref="StaticSpriteView.Scale"/>.
    /// </summary>
    ...
    {
        /// <summary>
        /// Don't automatically scale the sprite. The sprite can still be scaled via <see cref="StaticSpriteView.Scale"/>
        /// </summary>
        None,

        /// <summary>
        /// Scales the sprite down so that it fits within the control. Does not scale the sprite up. Keeps the same
        /// aspect ratio. This automatic scaling is done before applying <see cref="StaticSpriteView.Scale"/>.
        /// </summary>
        Fit,

        /// <summary>
        /// Scale the sprite up or down so that it fills the whole control. Keeps the same aspect ratio. This
        /// automatic scaling is done before applying <see cref="StaticSpriteView.Scale"/>.
        /// </summary>
        Fill
    }
    ...
    public void SetEntity(EntityUid? uid)
    {
        ...
        // Create a sprite copy
        var cached = EntMan.Spawn();
        var cachedComp = EntMan.EnsureComponent<SpriteComponent>(cached);
        _cachedSprite = (cached, cachedComp);

        SpriteSystem?.CopySprite((uid.Value, sprite), _cachedSprite.Value.AsNullable());
    ...
    private void UpdateSize()
    {
        if (SpriteSystem == null
            || !ResolveEntity(out _, out var sprite, out _))
            return;

        var spriteBox = SpriteSystem.CalculateBounds(sprite.Value, default, _worldRotation ?? Angle.Zero, _eyeRotation)
            .CalcBoundingBox();
    ...
    protected override void Draw(IRenderHandle renderHandle)
    {
        ...
        var offset = SpriteOffset
            ? Vector2.Zero
            : - (-_eyeRotation).RotateVec(_cachedSprite.Value.Comp.Offset * _scale) * new Vector2(1, -1) * EyeManager.PixelsPerMeter;
    ...
    private bool ResolveEntity(
        out EntityUid uid,
        [NotNullWhen(true)] out Entity<SpriteComponent>? sprite,
